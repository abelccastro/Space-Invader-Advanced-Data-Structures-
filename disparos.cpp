...
bool elimina_bala_objeto(struct NAVE& N, struct NAVE& E, struct Balas B[]){///elimina bala q choca con objeto y con el vida>0 evita a los enemigos eliminados
    if ( N.n_disp > 0 && N.n_disp < N.max_disp){///verifica que haiga balas disponibles
            for ( int cont = 1; cont <= N.n_disp; cont++){///recorrer todo el arreglo de balas
                if(colicion(E.x , E.y , E.ancho_p , E.alto_p,B[cont].x , B[cont].y , N.ancho_b , N.alto_b)&& E.vida > 0){///vida>0 para que no choque la bala con enemigos invisibles
                    eliminar(B,N.n_disp,cont);///ESTO ES PARA ELIIMINAR LA BALA MEGARMA
                    E.vida--;
                    cout<<"vida restada:   "<<E.vida<<endl;
                    return true;
                }
            }
            //cout<<"retrono falso"<<endl;
            return false;
    }
}
